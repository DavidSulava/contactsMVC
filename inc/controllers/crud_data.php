...
class DataCrude{
    ...
    public static function addContact( $request=[] ){
        ...
        $model   = App::db_model();
        $contact = $model->fetch("SELECT * FROM Contacts WHERE id=? AND c_id=?", [ $user_id ,  $contact_id ]);

        if( $contact )
            return  header("Location: home?$getParameters&$msgErr");

        $respR = $model->request("INSERT INTO Contacts  (id, c_id) VALUES ( ?, ? ) ", [ $user_id, $contact_id ]);
    ...
    public static function delete( $request=[] ){
        ...
        if($contact)
            $qStore = $model->request("DELETE FROM Contacts WHERE id=? AND c_id=? ", [ $user_id, $contact_id ] );
    ...
    public static function deleteAll( $request=[] ){
        ...
        $model    = App::db_model( );
        $contacts = $model->fetchAll("SELECT * FROM Contacts WHERE id=?", [ $user_id ] );

        if( $contacts && count( $contacts ) > 0 )
            $contacts = $model->fetchAll("DELETE FROM Contacts WHERE id=?", [ $user_id ] );
